...
public class MultiLineString<C extends XY> extends Geometry {
	...
	public boolean equals(Object o) {
		...
		if (lineStrings == null && that.lineStrings != null) return false;
		if (lineStrings.size() != that.lineStrings.size()) return false;

		for (int i = 0; i < lineStrings.size(); i++) {
			if (!lineStrings.get(i).equals(that.lineStrings.get(i))) return false;
		}
	...
	public static MultiLineString<? extends XY> unMarshall(byte[] bytes) {
		...
		if (geometryType == GeometryType.MULTILINESTRING.getCode()) return unMarshall(XY.class, buffer);
		if (geometryType == GeometryType.MULTILINESTRINGZ.getCode()) return unMarshall(XYZ.class, buffer);
		if (geometryType == GeometryType.MULTILINESTRINGM.getCode()) return unMarshall(XYM.class, buffer);
		if (geometryType == GeometryType.MULTILINESTRINGZM.getCode()) return unMarshall(XYZM.class, buffer);
		return null;
	}
